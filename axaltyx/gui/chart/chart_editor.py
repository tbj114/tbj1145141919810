...
class ChartEditor:
    ...
    def __init__(self, root):
        ...
        self.root.title(self.i18n.t("app.app_name") + " - " + self.i18n.t("app.chart_editor.title"))
        ...
        # 创建左侧控制面板
        self.control_panel = ttk.LabelFrame(self.main_frame, text=self.i18n.t("app.chart_editor.control_panel"))
        self.control_panel.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5, expand=False)
        
        # 创建右侧图表区域
        self.chart_frame = ttk.LabelFrame(self.main_frame, text=self.i18n.t("app.chart_editor.chart_preview"))
    ...
    def _init_chart_type(self):
        """初始化图表类型选择"""
        ttk.Label(self.control_panel, text=self.i18n.t("app.chart_editor.chart_type")).pack(padx=10, pady=5, anchor=tk.W)
        
        self.chart_type = tk.StringVar()
        # 内部图表类型标识符
        self.chart_type_ids = [
            "bar", "line", "scatter", "histogram", "boxplot", "pie",
            "heatmap", "network", "sankey", "wordcloud", "multi_panel"
        ]
        # 显示的图表类型名称（翻译后的）
        chart_types = [
            self.i18n.t("app.chart_types.bar"),
            self.i18n.t("app.chart_types.line"),
            self.i18n.t("app.chart_types.scatter"),
            self.i18n.t("app.chart_types.histogram"),
            self.i18n.t("app.chart_types.boxplot"),
            self.i18n.t("app.chart_types.pie"),
            self.i18n.t("app.chart_types.heatmap"),
            self.i18n.t("app.chart_types.network"),
            self.i18n.t("app.chart_types.sankey"),
            self.i18n.t("app.chart_types.wordcloud"),
            self.i18n.t("app.chart_types.multi_panel")
        ]
    ...
    def _init_data_input(self):
        """初始化数据输入"""
        ttk.Label(self.control_panel, text=self.i18n.t("app.chart_editor.data_input")).pack(padx=10, pady=5, anchor=tk.W)
        ...
        # 数据导入按钮
        self.import_button = ttk.Button(self.control_panel, text=self.i18n.t("app.chart_editor.import_data"), command=self.import_data)
        self.import_button.pack(padx=10, pady=5, fill=tk.X)
    
    def _init_chart_params(self):
        """初始化图表参数"""
        ttk.Label(self.control_panel, text=self.i18n.t("app.chart_editor.chart_params")).pack(padx=10, pady=5, anchor=tk.W)
        
        # 标题
        ttk.Label(self.control_panel, text=self.i18n.t("app.chart_editor.title_label")).pack(padx=10, pady=2, anchor=tk.W)
        ...
        # X轴标签
        ttk.Label(self.control_panel, text=self.i18n.t("app.chart_editor.xlabel")).pack(padx=10, pady=2, anchor=tk.W)
        ...
        # Y轴标签
        ttk.Label(self.control_panel, text=self.i18n.t("app.chart_editor.ylabel")).pack(padx=10, pady=2, anchor=tk.W)
        ...
        self.interactive_check = ttk.Checkbutton(self.control_panel, text=self.i18n.t("app.chart_editor.enable_interaction"), variable=self.interactive_var)
        self.interactive_check.pack(padx=10, pady=5, anchor=tk.W)
    
    def _init_multi_panel(self):
        """初始化多面板设置"""
        self.multi_panel_frame = ttk.LabelFrame(self.control_panel, text=self.i18n.t("app.chart_editor.multi_panel_settings"))
        self.multi_panel_frame.pack(padx=10, pady=5, fill=tk.X)
        
        # 行数
        ttk.Label(self.multi_panel_frame, text=self.i18n.t("app.chart_editor.rows")).pack(padx=10, pady=2, anchor=tk.W)
        ...
        # 列数
        ttk.Label(self.multi_panel_frame, text=self.i18n.t("app.chart_editor.cols")).pack(padx=10, pady=2, anchor=tk.W)
    ...
    def _init_buttons(self):
        """初始化按钮"""
        # 绘制按钮
        self.draw_button = ttk.Button(self.control_panel, text=self.i18n.t("app.chart_editor.draw_chart"), command=self.draw_chart)
        self.draw_button.pack(padx=10, pady=10, fill=tk.X)
        
        # 保存按钮
        self.save_button = ttk.Button(self.control_panel, text=self.i18n.t("app.chart_editor.save_chart"), command=self.save_chart)
    ...
    def draw_chart(self):
        ...
        # 获取选中的图表类型索引
        chart_type_index = self.chart_type_combo.current()
        chart_type_id = self.chart_type_ids[chart_type_index]
        interactive = self.interactive_var.get()
        
        if chart_type_id == "bar":
            self.chart = BarChart()
            self.chart.plot(data)
        elif chart_type_id == "line":
            self.chart = LineChart()
            self.chart.plot(data)
        elif chart_type_id == "scatter":
        ...
        elif chart_type_id == "multi_panel":
