...
class ExploreDialog(ArcoDialog):
    """探索分析对话框"""
    ...
    def __init__(self, parent=None, dataset=None):
        ...
        self.set_title(self.i18n.t("sidebar.explore"))
    ...
    def _init_ui(self):
        ...
        self._create_plot_checkboxes(plot_layout)
        plot_widget.setLayout(plot_layout)
        options_tabs.addTab(plot_widget, self.i18n.t("dialogs.common.charts"))
        
        # 异常值选项
        outlier_widget = QWidget()
        outlier_layout = QVBoxLayout()
        outlier_layout.setSpacing(12)
        self._create_outlier_checkboxes(outlier_layout)
        outlier_widget.setLayout(outlier_layout)
        options_tabs.addTab(outlier_widget, "异常值检测")
    ...
    def _create_statistics_checkboxes(self, parent_layout):
        ...
        self.check_kurtosis = QCheckBox("峰度")
        ...
        self.check_skewness = QCheckBox("偏度")
    ...
    def _create_plot_checkboxes(self, parent_layout):
        """创建图表复选框"""
        group = QGroupBox("图表")
        group_layout = QVBoxLayout()
        group_layout.setSpacing(10)
        ...
        self.check_histogram.toggled.connect(self._update_plot_options)
        group_layout.addWidget(self.check_histogram)
        
        self.check_boxplot = QCheckBox("箱线图")
        self.check_boxplot.setChecked(True)
        self.check_boxplot.toggled.connect(self._update_plot_options)
        group_layout.addWidget(self.check_boxplot)
        
        self.check_normality = QCheckBox("正态性检验图")
        self.check_normality.setChecked(True)
        self.check_normality.toggled.connect(self._update_plot_options)
        group_layout.addWidget(self.check_normality)
        
        group.setLayout(group_layout)
        parent_layout.addWidget(group)
    
    def _create_outlier_checkboxes(self, parent_layout):
        """创建异常值检测复选框"""
        group = QGroupBox("异常值检测")
        group_layout = QVBoxLayout()
        group_layout.setSpacing(10)
        
        self.check_identify_outliers = QCheckBox("识别异常值")
        self.check_identify_outliers.toggled.connect(self._update_outlier_options)
        group_layout.addWidget(self.check_identify_outliers)
        
        self.check_exclude_outliers = QCheckBox("排除异常值")
        self.check_exclude_outliers.toggled.connect(self._update_outlier_options)
        group_layout.addWidget(self.check_exclude_outliers)
    ...
    def _update_plot_options(self):
        """更新图表选项"""
        self.plot_options['histogram'] = self.check_histogram.isChecked()
        self.plot_options['boxplot'] = self.check_boxplot.isChecked()
        self.plot_options['normality_plot'] = self.check_normality.isChecked()
    
    def _update_outlier_options(self):
        """更新异常值选项"""
        self.outlier_options['identify_outliers'] = self.check_identify_outliers.isChecked()
        self.outlier_options['exclude_outliers'] = self.check_exclude_outliers.isChecked()
    ...
    def _on_reset(self):
        ...
        # 重置异常值选项
        self.check_identify_outliers.setChecked(False)
        self.check_exclude_outliers.setChecked(False)
    ...
    def _on_ok(self):
        ...
        result_data = {
            ...
            'outlier_options': self.outlier_options.copy()
        }
    ...
    def _get_variable_data(self, var_name):
        ...
        # 添加一些异常值
        if self.outlier_options['identify_outliers']:
            data[0] = mean + std * 3.5
            data[1] = mean - std * 3.5
